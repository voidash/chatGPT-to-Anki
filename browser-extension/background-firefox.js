// Chat to Anki Flashcards - Background Script (Firefox Compatible)
...
browserAPI.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === 'addToContext') {
    ...
    browserAPI.storage.local.get(['contextData'], (result) => {
      ...
      browserAPI.storage.local.set({ contextData: currentContext }, () => {
        console.log('Context item added:', contextItem);
        
        // Show notification (Firefox compatible)
        if (browserAPI.notifications) {
          browserAPI.notifications.create({
            type: 'basic',
            iconUrl: browserAPI.runtime.getURL('icon-48.png'),
            title: 'Context Added',
...
browserAPI.runtime.onMessage.addListener((request, sender, sendResponse) => {
  switch (request.action) {
    ...
    case 'downloadAnkiDeck':
      // Handle Anki deck download (Firefox compatible)
    ...
    case 'getStoredData':
      browserAPI.storage.local.get(request.keys, (result) => {
        sendResponse(result);
      });
      return true;
...
async function handleContextFlashcardGeneration(request, sendResponse) {
  try {
    ...
    const result = await new Promise(resolve => {
      browserAPI.storage.local.get(['contextData', 'settings'], resolve);
    });
    ...
    const chatGPTTab = await browserAPI.tabs.create({ url: 'https://chatgpt.com' });
    
    // Wait for the tab to load and inject the prompt with full automation
    setTimeout(() => {
      if (browserAPI.tabs.executeScript) {
        // Firefox Manifest V2
        browserAPI.tabs.executeScript(chatGPTTab.id, {
          code: `(${injectAndProcessContextPrompt.toString()})(${JSON.stringify(finalPrompt)});`
        });
      } else {
...
function injectAndProcessContextPrompt(prompt) {
  ...
  function openConfigurationPage() {
    ...
    setTimeout(() => {
      ...
      const configUrl = browserAPI.runtime.getURL('config.html');
